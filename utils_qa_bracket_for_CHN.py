...
def postprocess_qa_predictions(
    ...
    ## 괄호 span하는 것 아니면 제외 코드 ########################################3
    # 위키 단어-괄호 안의 단어(key-value 저장값 불러오기)
    pickle_name = f"wiki_bracket_nosplit.bin"
    path = os.path.join("/opt/ml/", pickle_name)
    with open(path, "rb") as file:
        dict_wiki = pickle.load(file) 
    print("-------------------------------------dict load---------------------------")
        ...
        if not version_2_with_negative:
            ...
            if "CHN" in text:
                for dict_k in dict_wiki:
                    check=False
                    tmp=re.compile('[ぁ-ゔァ-ヴー々〆〤一-龥]+').sub('',dict_wiki[dict_k])
                    if len(tmp)!=len(dict_wiki[dict_k]):
                        if dict_k in context and len(dict_k.split())!=0:
                            tmp_text = text[0:text.find("[CHN]")]
                            ccc = dict_k.split()[len(dict_k.split())-1]
                            if tmp_text in dict_k and len(tmp_text.split())!=0:
                                new_text = tmp_text + "(" + dict_wiki[dict_k] + ")" + text[text.find("[CHN]")+5:]
                                check=True
                                break
            if len(new_text)==0:
                new_text=text
