    ...
    def prepare_in_batch_negative(self, dataset, num_neg, tokenizer, is_train):
        ...
        if is_train:
            train_dataset = TensorDataset(
                p_seqs["input_ids"],
                p_seqs["attention_mask"],  # p_seqs['token_type_ids'],
                q_seqs["input_ids"],
                q_seqs["attention_mask"],  # q_seqs['token_type_ids'],
            )
        ...
        for _ in train_iterator:
            ...
            with tqdm(self.train_dataloader, unit="batch") as tepoch:
                ...
                for i, batch in enumerate(tepoch):
                    ...
                    train_correct += (
                        torch.argmax(sim_scores, dim=1).eq(targets).sum().item()
                    )
                    ...
                    if count_iteration == valid_check_period:
                        ...
                        with torch.no_grad():
                            ...
                            with tqdm(
                                self.valid_dataloader, unit="batch"
                            ) as tepoch_val:
